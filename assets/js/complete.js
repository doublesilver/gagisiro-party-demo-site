/* =============================================
   COMPLETE PAGE — depends on common.js (API_BASE, esc, fmtPrice, revealPage, fetchSiteContent, sharePage, DEFAULT_PRICES)
   ============================================= */
    ...
    if (rawPricing) {
      ...
      Object.keys(pricing).forEach((key) => {
        ...
        newPrices[key] = {
          ...
          part2_base:
            p.part2_base != null ? Number(p.part2_base) : globalP2Base,
          part2_discount:
...
function renderComplete() {
  if (data) {
    ...
    if (data.totalPrice != null && data.totalPrice > 0) {
      ...
      const price =
        data.price ||
        (PRICES[data.branch] && PRICES[data.branch][data.gender]) ||
        0;
    ...
    const rows = [
        ...
        ? [
            {
              key: "2부참여",
              val:
                data.part2pay === "prepay"
                  ? "사전결제"
                  : data.part2pay === "onsite"
                    ? "현장결제"
                    : data.part2pay,
            },
          ]
      ...
      ...(data.discountAmount > 0
        ? [{ key: "할인금액", val: "-" + fmtPrice(data.discountAmount) }]
        : []),
...
(async function () {
  try {
    ...
    if (data.instagram_id) {
      ...
      if (link) {
        link.href =
          "https://www.instagram.com/" + encodeURIComponent(data.instagram_id);
        const textNode = link.childNodes;
        for (let i = 0; i < textNode.length; i++) {
          if (
            textNode[i].nodeType === 3 &&
            textNode[i].textContent.includes("@")
          ) {
            textNode[i].textContent = textNode[i].textContent.replace(
              /@[\w.]+/,
              "@" + data.instagram_id,
            );
            break;
